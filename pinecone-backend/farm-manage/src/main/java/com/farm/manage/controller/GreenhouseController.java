...
public class GreenhouseController extends BaseController
{
    ...
    @PreAuthorize("@ss.hasPermi('manage:greenhouse:getAll')")
    @Log(title = "大棚信息", businessType = BusinessType.OTHER)
    @GetMapping("/getAll")
    public AjaxResult getAllGreenhouse()
    {
        List<Greenhouse> list = greenhouseService.getAllGreenhouse();
        return success(list);
    }
