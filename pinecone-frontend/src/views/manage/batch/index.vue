<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="种质名称" prop="germplasmId">
        <el-select
    ...
    >
      ...
      <el-table-column label="种质名称" align="center" prop="germplasmId">
        <template #default="scope">
          <span>{{ getGermplasmName(scope.row.germplasmId) }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="开始时间"
...
import {
  ...
  addBatch,
  updateBatch,
} from "@/api/manage/batch";
import { listGermplasm } from "@/api/manage/germplasm";
...
/** 获取种质下拉框数据 */
function getGermplasmOptions() {
  listGermplasm().then((response) => {
    germplasmOptions.value = response.rows;
  });
}

/** 根据ID获取种质名称 */
function getGermplasmName(germplasmId) {
  const type = germplasmOptions.value.find(
    (item) => item.germplasmId === germplasmId
  );
  return type ? type.germplasmName : "";
}
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除作物批次编号为"' + _batchIds + '"的数据项？')
    .then(function () {
