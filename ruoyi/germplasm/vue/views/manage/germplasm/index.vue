<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="germplasmList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:germplasm:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:germplasm:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="germplasmRef" :model="form" :rules="rules" label-width="80px">
      </el-form>
      <template #footer>
...
import { listGermplasm, getGermplasm, delGermplasm, addGermplasm, updateGermplasm } from "@/api/manage/germplasm";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除种质系统编号为"' + _germplasmIds + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('manage/germplasm/export', {
    ...queryParams.value
