  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="materialInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:materialInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:materialInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="materialInfoRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="农资类别" prop="materialTypeId">
          <el-input v-model="form.materialTypeId" placeholder="请输入农资类别" />
        </el-form-item>
        <el-form-item label="计量单位" prop="measureUnit">
...
import { listMaterialInfo, getMaterialInfo, delMaterialInfo, addMaterialInfo, updateMaterialInfo } from "@/api/manage/materialInfo";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除农资信息编号为"' + _materialIds + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('manage/materialInfo/export', {
    ...queryParams.value
